...
export interface Analysis extends Summary { capture_source?: 'SYNTHETIC_FIXTURE' | 'UNVERIFIED'; capture_sha256: string; capture_size: number; capture_duration: number; analysis_status: string; protocol_observations: { counts: Record<string,number>; ike_messages: Ike[]; spi_changes: number; malformed_packets: number; unsupported_packets: number; ah_next_headers: number[] }; security_associations: SA[]; traffic_predictions: Prediction[]; findings: Finding[]; threat_matrix: Threat[]; limitations: string[]; report_references: Record<string,string>; telemetry_provenance: string[]; revision: number }
