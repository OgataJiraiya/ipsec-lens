...
the normal local application. The preview flag controls presentation; it is not
...
The frontend flag is baked into the Docker build, so the runtime value is documented
for clarity but changing it alone does not change the compiled UI. Rebuild to change
the preview mode. Do not set `IPSECLENS_ABSTENTION`; the canonical threshold remains
`0.60`.
...
capture, VPN lab, or Docker commands.
...
telemetry. The UI hides export and deletion, but the underlying API routes remain
available for the normal application and are not an access-control boundary.
